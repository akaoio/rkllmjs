...
export class CLIRunner {
  ...
  async run(args: string[]): Promise<void> {
    ...
    console.log(
      `🔧 Runtime: ${runtime.type} v${runtime.version} ${runtime.type === 'node' ? '(Primary)' : '(Alternative)'}\n`
    );

    // Show runtime warning for experimental runtimes
    if (this.detector.isExperimental()) {
      console.log(
        `⚠️  Warning: ${runtime.type} is experimental. Consider using Node.js for production.`
      );
      ...
      for (const item of items) {
        ...
        if (item.isDirectory()) {
          ...
          for (const subItem of subItems) {
            ...
            console.log(
              `      ${subItem.isDirectory() ? '📁' : '📄'} ${subItem.name} (${subStat.size} bytes)`
            );
  ...
  private showHelp(): void {
    ...
    console.log(`📖 Usage:`);
    console.log(
      `   ${prefix} cli-runner.js pull <repo> <model>       - Download specified RKLLM model + essential technical files`
    );
    console.log(
      `   ${prefix} cli-runner.js list                      - List all downloaded models`
    );
