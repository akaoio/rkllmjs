...
import { RKLLMClient, type RKLLMClientConfig } from './rkllm-client.js';
import { RKLLMInputType, RKLLMStatusCode, RKLLMError } from '../rkllm-types/rkllm-types.js';
import {
...
describe('RKLLM Client - Production Tests', () => {
  ...
  describe('Configuration', () => {
    it('should create client with production config', () => {
      ...
      try {
        ...
        testLogger.info('Production config test completed with real model', {
          modelPath: realModelPath,
        });
      } catch (error) {
    ...
    it('should initialize with real model and native bindings', async () => {
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes('Test model not found')) {
          ...
          testLogger.info(
            '   Download a model first: npm run cli pull dulimov/Qwen2.5-VL-7B-Instruct-rk3588-1.2.1 Qwen2.5-VL-7B-Instruct-rk3588-w8a8-opt-1-hybrid-ratio-0.5.rkllm'
          );
          return;
        }
  ...
  describe('Performance', function () {
    beforeEach(async () => {
      ...
      if (!areNativeBindingsAvailable()) {
        testLogger.info(
          '⚠️  Skipping performance test setup - native RKLLM bindings not available'
        );
        return;
      }
